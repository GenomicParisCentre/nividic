...
/**
 * This class define a processor to read expression matrix.
 * @author Laurent Jourdren
 */
public class ReadMatrix implements NividicLocalWorker {
  ...
  public String[] outputNames() {
    return new String[] {"matrix", "translator"};
  }
  ...
  public Map<String, DataThing> execute(Map<String, DataThing> inputs)
    ...
    try {
      ...
      SimpleExpressionMatrixReader reader =
          new SimpleExpressionMatrixReader(new File(filename));

      final ExpressionMatrix matrix = reader.read();
      final Translator translator = reader.getTranslator();

      NividicDataThingAdapter outputs = new NividicDataThingAdapter();

      outputs.putExpressionMatrix("matrix", matrix);
      outputs.putTranslator("translator", translator);

...
/**
 * This class describe the experiment design.
 * 
 * @author Laurent Jourdren
 */
public class DesignImpl implements Design, Serializable {
	static final long serialVersionUID = 7308664633759590547L;
	...
	private Map<String, Integer> scanLabelSettingsFields = new HashMap<String, Integer>();
	private List<String> scanLabelSettingsOrder = new ArrayList<String>();
	private Map<String, String> scanLabelSettingsData = new HashMap<String, String>();
	...
	public void renameLabel(final String oldName, final String newName) {
		...
		if (isLabel(newName))
			throw new NividicRuntimeException(
					"the new label name already exists");
	 ...
	 * 
	 * @param oldName
	 *            Old name of the slide
	 * @param newName
	 *            New name of the slide
	 */
	public void renameSlide(final String oldName, final String newName) {
		...
		if (isSlide(newName))
			throw new NividicRuntimeException(
					"the new slide name already exists");
	 ...
	 * 
	 * @param slideName
	 *            The name of the slide to extract
	 ...
	 * 
	 * @param index
	 *            Index of the slide in the design
	 * @return a slide object
	 */
	...
	public void setTarget(final String slide, final String label,
		...
		if (!this.samples.isSample(sample))
			throw new NividicRuntimeException(
					"The sample name doesn't exists: " + sample);
	 ...
	 * 
	 * @param name
	 *            Name of the description field to test
	...
	public void addDescriptionField(final String name) {
		...
		if (name == null)
			throw new NividicRuntimeException(
					"The description field can't be null");
	 ...
	 * 
	 * @param oldName
	 *            Old name of the description field
	 * @param newName
	 *            New name of the description field
	 */
	public void renameDescriptionField(final String oldName,
		...
		final int index = Collections.binarySearch(this.descriptionsOrder,
				oldName);
	 ...
	 * 
	 * @param name
	 *            Name of the descritpion field to remove
	 */
	 ...
	 *            Description field
	 * @param value
	 *            of the description to set
	 */
	public void setDescription(final String slide,
			final String descriptionField, final String value) {
	 ...
	 */
	public String getDescription(final String slide,
			final String descriptionField) {
		...
		if (!isDescriptionField(descriptionField))
			throw new NividicRuntimeException(
					"The description name doesn't exists");
	...
	public void setSourceFormat(final String slideName, final String formatName) {
		...
		if (formatName == null)
			...
			BioAssayFormat format = BioAssayFormatRegistery
					.getBioAssayFormat(formatName);
	...
	public String getScanLabelSetting(final String slide, final String label,
		...
		if (!isScanLabelSetting(setting))
			throw new NividicRuntimeException(
					"The scan setting name doesn't exists");

		return this.scanLabelSettingsData.get(createkeyLabelScanLabelSetting(
				slide, label, setting));
	}
	...
	public void setScanLabelSetting(final String slide, final String label,
		...
		this.scanLabelSettingsData.put(createkeyLabelScanLabelSetting(slide,
				label, setting), value);
	}
	 ...
	 * 
	 * @param oldName
	 *            Old name of the scan label setting
	 * @param newName
	 *            New name of the scan label setting
	 */
	public void renameScanLabelSetting(final String oldName,
			final String newName) {
		...
		final int index = Collections.binarySearch(this.scanLabelSettingsOrder,
				oldName);
	 ...
	 * 
	 * @param setting
	 *            Setting name to remove
	 */
	public void removeScanLabelSetting(final String setting) {

		if (setting == null)
			throw new NividicRuntimeException(
					"Scan label setting name can't be null");

		if (!isScanLabelSetting(setting))
			throw new NividicRuntimeException(
					"the scan label setting doesn't exists");
	...
	private void addConstructorHistoryEntry() {
		...
		final HistoryEntry entry = new HistoryEntry("Create Design (#"
				+ getBiologicalId() + ")", HistoryActionType.CREATE,
				"SlideNumbers=" + getSlideCount() + ";LabelNumber="
						+ getLabelCount(), HistoryActionResult.PASS);
